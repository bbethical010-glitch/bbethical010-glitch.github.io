      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-12">
          {CHANGELOG.map((release, idx) => (
            <div 
              key={idx} 
              className="relative border-l-4 border-purple ml-4 pl-8 py-4"
              data-rv="up"
            ...
            >
              ...
              <div className="neo-card bg-surface p-6">
                <div className="flex flex-wrap items-center gap-4 mb-4 border-b-2 border-purple pb-4">
                  ...
                  <span className={`px-2 py-1 text-xs font-bold uppercase bg-${release.tagColor} text-bg border border-${release.tagColor}`}>{release.tag}</span>
                </div>
                <ul className="space-y-3">
                  {release.changes.map((change, cIdx) => (
                    <li key={cIdx} className="font-oswald text-text text-base flex items-start">
                      <span className="text-gold mr-3 mt-1.5 text-xs">■</span>{change}
                    </li>
                  ))}
                </ul>
              </div>
